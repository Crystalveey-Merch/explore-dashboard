...
export const ReviewTours = ({ allTourBookings }: { allTourBookings: any[] }) => {
    ...
    useEffect(() => {
        setLoading(true);
        // set bookings of type "Tour Packages" and status "pending" to state
        setTourBookings(allTourBookings.filter((booking: { type: string, status: string }) => booking.type === "Tour Packages" && booking.status === "pending"));
        setLoading(false);
    }, [allTourBookings]);
