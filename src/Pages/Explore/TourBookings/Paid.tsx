...
export const PaidTours = ({ allTourBookings }: { allTourBookings: any[] }) => {
    ...
    useEffect(() => {
        setLoading(true);
        // set bookings of type "Tour Packages"  and status "confirmed" to state
        setTourBookings(allTourBookings.filter((booking: { type: string, status: string }) => booking.type === "Tour Packages" && booking.status === "confirmed"));
        setLoading(false);
    }, [allTourBookings]);
