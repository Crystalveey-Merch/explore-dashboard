...
export const CancelledRetreats = () => {
    ...
    useEffect(() => {
        const fetchTravelBookings = async () => {
            ...
            setRetreatBookings(bookings.filter((booking: { type: string, status: string }) => booking.type === "Retreats Packages" && booking.status === "cancelled"));
            setLoading(false);
        }
        fetchTravelBookings()
    }, [])
