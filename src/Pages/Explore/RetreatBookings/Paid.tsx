...
export const PaidRetreats = () => {
    ...
    useEffect(() => {
        const fetchTravelBookings = async () => {
            ...
            // set bookings of type "Retreats Packages" and status "confirmed" to state
            setRetreatBookings(bookings.filter((booking: { type: string, status: string }) => booking.type === "Retreats Packages" && booking.status === "confirmed"));
            setLoading(false);
        }
        fetchTravelBookings()
    }, [])
