...
export const RefundedRetreats = () => {
    ...
    useEffect(() => {
        const fetchTravelBookings = async () => {
            ...
            // set bookings of type "Retreats Packages" and status "refunded" to state
            setRetreatBookings(bookings.filter((booking: { type: string, status: string }) => booking.type === "Retreats Packages" && booking.status === "refunded"));
            setLoading(false);
        }
        fetchTravelBookings()
    }, [])
