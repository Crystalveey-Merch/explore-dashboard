...
export const ReviewActivities = () => {
    ...
    useEffect(() => {
        const fetchTravelBookings = async () => {
            ...
            setActivitiesBookings(bookings.filter((booking: { type: string, status: string }) => booking.type === "Exciting Activities" && booking.status === "pending"));
            setLoading(false);
        }
        fetchTravelBookings()
    }, [])
