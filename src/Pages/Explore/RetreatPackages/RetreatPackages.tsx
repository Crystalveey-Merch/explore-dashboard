...
export const RetreatPackages = () => {
    ...
    const handleDelete = async (id: string, title: string) => {
        if (window.confirm(`Are you sure you want to delete "${title}"?`)) {
            try {
                await deleteDoc(doc(db, "retreatPackages", id));

                // Update the state after successful deletion
                const newTravelPackages = retreatPackages.filter(
                    (travelPackage) => travelPackage.id !== id
                );
                setRetreatPackages(newTravelPackages);
    ...
    useEffect(() => {
        if (sort === "asc" && activeTab === "packageName") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => a.title.localeCompare(b.title)));
        } else if (sort === "desc" && activeTab === "packageName") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => b.title.localeCompare(a.title)));
        } else if (sort === "asc" && activeTab === "date") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => formatDateToString(a.startDate) - formatDateToString(b.startDate)));
        } else if (sort === "desc" && activeTab === "date") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => formatDateToString(b.startDate) - formatDateToString(a.startDate)));
        } else if (sort === "asc" && activeTab === "price") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => a.price - b.price));
        } else if (sort === "desc" && activeTab === "price") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => b.price - a.price));
        } else if (sort === "asc" && activeTab === "duration") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => a.duration - b.duration));
        } else if (sort === "desc" && activeTab === "duration") {
            setDisplayedRetreatPackages([...displayedRetreatPackages].sort((a, b) => b.duration - a.duration));
        } else {
            setDisplayedRetreatPackages(retreatPackages);
        }
    ...
    useEffect(() => {
        setDisplayedRetreatPackages(retreatPackages.filter((travelPackage) => travelPackage.title.toLowerCase().includes(search.toLowerCase())));
    }, [search, retreatPackages]);
    ...
    return (
        <div className="px-10 py-14 flex flex-col gap-20 xl:px-6 xl:w-[calc(100vw-10px)] lg:gap-16 md:gap-12 md:w-[100vw] sm:gap-9">
            <div className="flex justify-between items-center sm:flex-col sm:items-start sm:gap-3">
                ...
                <NavLink to="/explore/retreat-packages/add">
                    <BlueButton
                        className="lg:py-1.5"
                        label={
            ...
            >
                <div className="flex justify-end pt-5 px-4">
                    <SearchInput search={search} setSearch={setSearch} placeholder="Search for a package" />
                </div>
                <div className="relative w-full lg:overflow-x-scroll xl:overflow-y-hidden xl:rounded-lg">
                    <table className="w-full text-sm text-left text-gray-500">
                        ...
                        {displayedRetreatPackages.length === 0 ? (
                            ...
                            <>
                                {displayedRetreatPackages.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((travelPackage) => (
                                    <tbody key={travelPackage.id} >
                                        <tr className="bg-white border-b hover:bg-gray-50 ">
                                            ...
                                            <td className="flex items-center px-6 py-4 space-x-3">
                                                ...
                                                <NavLink to={`/explore/retreat-packages/edit/${travelPackage.id}`} className="font-medium my-auto text-blue-600 dark:text-blue-500 hover:underline">Edit</NavLink>
                                                <button
                ...
                <div className="p-2">
                    <TablePagination
                        component="div"
                        count={displayedRetreatPackages.length}
                        page={page}
