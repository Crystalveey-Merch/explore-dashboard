...
export const Review = ({ allTravelBookings }: { allTravelBookings: any[] }) => {
    ...
    useEffect(() => {
        setLoading(true);
        // set bookings of type "Promoted Travel Package" and status "pending" to state
        setTravelBookings(allTravelBookings.filter((booking: { type: string, status: string }) => booking.type === "Promoted Travel Package" && booking.status === "pending"));
        setLoading(false);
    }, [allTravelBookings])
        ...
        <div className="px-10 py-7 flex flex-col gap-10 xl:px-6 lg:gap-16 md:gap-12 sm:px-4 sm:gap-9">
            <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2.5 items-center">
                    ...
                    <Link to="/explore/travel-bookings" className="text-[rgb(33,43,54)] text-sm font-medium hover:underline">
                        Travel Bookings
                    </Link>
                    {/* a dot */}
                    <span className="h-1 w-1 rounded-full bg-[rgb(99,115,129)]">
